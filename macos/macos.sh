...
# USAGE: Run some macOS specific updates.

# I don't use macOS anymore so I've moved these to a separate file that doesn't get run
# But I'm keeping it around in case I need to use it again at some point.
# The last version of these dotfiles tested on macOS are pinned at release
###############################################################################
# Check whether we are on macOS or not
###############################################################################
if ! [[ $OSTYPE == "darwin" ]]; then
  printf -- "\n\033[31mThis script is meant for macOS only.\033[0m\n";
  exit 1
fi
...
# For MacOS we will want to install homebrew (https://brew.sh/)
